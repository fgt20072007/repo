...
function Structure:SetDifferential(BIAS, PRELOAD, MAX_TORQUE, TOP_SPEEDS, SHIFT_TIME)
	...
	if TOP_SPEEDS then
		...
		for i, v in TOP_SPEEDS do
			self.Ratios[i] = math_sign(i)
				* self.Engine.RedlineRPM
				* avgWheelRadius
				* Units.Studs_Meters
				* Units.RPM_KMH
				/ v
...
function Structure:Step(dt)
	...
	local isOutOfFuel = false
	local chassis = self.Chassis
	if chassis ~= nil then
		local fuelAmount = chassis:GetAttribute("FuelAmount")
		if type(fuelAmount) == "number" and fuelAmount <= 0 then
			isOutOfFuel = true
		end
	end
	...
	if self.TransmissionMode == "Auto" and self.RemainingShiftTime == 0 then
		...
		elseif
			self.Gear > 1
			and driveshaftRPM < 0.7 * redlineRPM / (self.Ratios[self.Gear - 1] / self.Ratios[self.Gear])
		then
			self.Shift = -1
...
return function(): Definition
	...
	self.Engine = Engine()
	self.Wheels = {
		Wheelstrut({ RayParameters = self.RayParameters, Side = false }),
		Wheelstrut({ RayParameters = self.RayParameters, Side = true }),
		Wheelstrut({ RayParameters = self.RayParameters, Side = false }),
		Wheelstrut({ RayParameters = self.RayParameters, Side = true }),
	}

	self.Brakes = {
		Brake({ Attachments = { self.Wheels[1] } }),
		Brake({ Attachments = { self.Wheels[2] } }),
		Brake({ Attachments = { self.Wheels[1] } }),
		Brake({ Attachments = { self.Wheels[2] } }),
		Brake({ Attachments = { self.Wheels[3] } }),
		Brake({ Attachments = { self.Wheels[4] } }),
	}
	...
	for _, Wheel in self.Wheels do
		table.insert(self.Components :: any, Wheel)
	end
	for _, Brake in self.Brakes do
		table.insert(self.Components :: any, Brake)
	end
