...
BackpackScript.EnabledChanged = Instance.new("BindableEvent")
...
function BackpackScript:SetBackpackEnabled(Enabled: boolean): ()
	...
	BackpackScript.EnabledChanged:Fire(Enabled)
	
	local HUD = PlayerGui:FindFirstChild("HUD") :: ScreenGui
	if HUD then
		local Controls = HUD:FindFirstChild("Controls") :: Frame
		if Controls then
