...
local CombatConfig = require(app.Shared.Data.CombatConfig)
local MovementStats = require(app.Shared.Data.MovementStats)

local SprintController = {}
SprintController.__index = SprintController

local function getSprintLockAttributeName()
	local clientConfig = CombatConfig.Client
	if type(clientConfig) == "table" and type(clientConfig.SprintLockAttribute) == "string" and clientConfig.SprintLockAttribute ~= "" then
		return clientConfig.SprintLockAttribute
	end

	return "CombatSprintLocked"
end
...
function SprintController:_canSprint()
	...
	if self:_isCombatSprintLocked() then
		return false
	end
...
function SprintController:_isCombatSprintLocked()
	if not self._character then
		return false
	end

	return self._character:GetAttribute(getSprintLockAttributeName()) == true
end
...
function SprintController:_syncCharacter(character)
	...
	self:_addConnection(character:GetAttributeChangedSignal(getSprintLockAttributeName()):Connect(function()
		if self:_isCombatSprintLocked() then
			self:_setSprinting(false)
			self:_setHumanoidWalkSpeed(MovementStats.DefaultWalkSpeed)
		end
...
function SprintController:Start()
	...
	ContextActionService:BindAction("SprintController_Sprint", function(...)
