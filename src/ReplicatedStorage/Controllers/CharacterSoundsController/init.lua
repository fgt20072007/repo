...
local Engine = RStorage:WaitForChild("ACS_MICTLAN")
...
local function checkMovement(humanoid: Humanoid): boolean
	...
	if humanoid.WalkSpeed > 0 and velocity > 0 and humanoid.FloorMaterial ~= Enum.Material.Air and humanoid.HipHeight >= 0 then
		return true
	end
	return false
end
...
local function PlaySound(character: Model, rootPart: Part, template: Sound, localMultiplier: number, multiplier: number, pitchMul: number)
...
function CharacterSoundsController:WrapCharacter(character: Model)
	...
	local RunningS = Instance.new("Sound")
	RunningS.Name = "Running"
	RunningS.Parent = rootPart
	local JumpingS = Instance.new("Sound")
	JumpingS.Name = "Jumping"
	JumpingS.Parent = rootPart
	local LandingS = Instance.new("Sound")
	LandingS.Name = "Landing"
	LandingS.Parent = rootPart
	rootPart:WaitForChild("Running").SoundId = ""
	rootPart:WaitForChild("Jumping").SoundId = ""
	rootPart:WaitForChild("Landing").SoundId = ""
	local CanPlayFootstepSound = character:WaitForChild("CharStats"):WaitForChild("CanRun")
			...
			if newState == Enum.HumanoidStateType.Landed then
				...
				if soundGroup then
					...
					if template then
						...
						CanPlayFootstepSound.Value = false
						task.delay(0.1, function()
							CanPlayFootstepSound.Value = true
						end)
					end
				end
			end
	...
	task.spawn(function()
		while humanoid.Health > 0 do
			...
			if checkMovement(humanoid) and velocity > 0.2 and CanPlayFootstepSound.Value == true then
						...
						elseif template:IsA("Folder") then
							for _, templateSound in pairs(template:GetChildren()) do
								PlaySound(
									...
									FootstepsConfig.LOCALPLAYER_VOLUME_MULTIPLIER * finalVelocity,
									FootstepsConfig.VOLUME_MULTIPLIER * finalVelocity,
									1 + (humanoid.WalkSpeed - ServerConfig.NormalWalkSpeed) * 0.02
								)
							end
						end
					end
...
local function wrapCharacter(character)
	task.spawn(function()
		CharacterSoundsController:WrapCharacter(character)
	end)
end

function CharacterSoundsController.Init(autoWrapPlayers: boolean): typeof(CharacterSoundsController)
