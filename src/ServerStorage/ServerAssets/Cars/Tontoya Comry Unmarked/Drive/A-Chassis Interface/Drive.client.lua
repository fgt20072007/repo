...
local _Tune = require(car:FindFirstChildOfClass("ModuleScript"))

local Packages = ReplicatedStorage:WaitForChild("Packages")

local Satchel = require(Packages:WaitForChild("Satchel", 10))
...
local _GPStickThrottle = 0
local _GPStickBrake = 0
local _GPTriggerThrottle = 0
local _GPTriggerBrake = 0

local function ApplyGamepadDriveInputs()
	if not _IsOn then
	...
	local throttleInput = math.max(_GPStickThrottle, _GPTriggerThrottle)
	local brakeInput = math.max(_GPStickBrake, _GPTriggerBrake)

	if brakeInput > throttleInput then
		_GBrake = brakeInput
		_GThrot = _Tune.IdleThrottle/100
	else
		_GThrot = math.max(_Tune.IdleThrottle/100, throttleInput)
		_GBrake = 0
	end
end
...
--Input Handler
local function IsInputAllowed(input: InputObject, isRobloxFunction:boolean)
	if UserInputService:GetFocusedTextBox() ~= nil then return false, true end
	local chatConfig = TextChatService:FindFirstChildOfClass("ChatInputBarConfiguration")
	if chatConfig and chatConfig.IsFocused then return false, true end
	local isTouch = UserInputService.TouchEnabled and input.UserInputType == Enum.UserInputType.Touch
	if GuiService.MenuIsOpen and not isTouch then return false, true end
	if _InControls then return false, true end
	if isRobloxFunction and input.UserInputType.Name:find("Gamepad") then return true, false end
	if isRobloxFunction then return false, false end
	return true, false
end

function DealWithInput(input,IsRobloxFunction)

	local allowed, shouldReset = IsInputAllowed(input, IsRobloxFunction)
	if allowed then --Ignore when UI/Chat Focus
		--Gamepad Digital Steer (fallback)
		...
		if input.UserInputType.Name:find("Gamepad") then
			--Gamepad Steering
			if input.KeyCode == _CTRL["ContlrSteer"] or input.KeyCode == Enum.KeyCode.Thumbstick1 then
				...
				--Gamepad Stick Drive: Thumbstick1 Y for throttle/brake
				if _IsOn and input.KeyCode == Enum.KeyCode.Thumbstick1 then
					local yDZone = math.min(.99, math.max(_Tune.Peripherals.ControlLDZone, _Tune.Peripherals.ControlRDZone)/100)
					local y = input.Position.Y

					if y < -yDZone then
						_GPStickThrottle = (-y - yDZone)/(1 - yDZone)
						_GPStickBrake = 0
					elseif y > yDZone then
						_GPStickBrake = (y - yDZone)/(1 - yDZone)
						_GPStickThrottle = 0
					else
						_GPStickThrottle = 0
						_GPStickBrake = 0
					...
					ApplyGamepadDriveInputs()
				end

				--Gamepad Throttle
			elseif _IsOn and input.KeyCode == _CTRL["ContlrThrottle"] then
				_GPTriggerThrottle = input.UserInputState == Enum.UserInputState.End and 0 or math.max(0, input.Position.Z)
				ApplyGamepadDriveInputs()

				--Gamepad Brake
			elseif input.KeyCode == _CTRL["ContlrBrake"] then
				_GPTriggerBrake = input.UserInputState == Enum.UserInputState.End and 0 or math.max(0, input.Position.Z)
				ApplyGamepadDriveInputs()
			end
		end
	elseif shouldReset then
		_GThrot = _Tune.IdleThrottle/100
		_GSteerT = 0
		_GBrake = 0
...
local Handbrake = Buttons:WaitForChild("Parking")
local Headlights = Buttons:WaitForChild("Headlights") :: GuiButton
local Horn = Buttons:WaitForChild("Horn") :: GuiButton

if UserInputService.TouchEnabled then
	Buttons.Visible = true

	GuiService.TouchControlsEnabled = false
	script.Parent:WaitForChild("Gauge").Visible = false
end
...
Headlights.MouseButton1Down:Connect(function()
	...
	lastCheck = now
	car.ToggleHeadlights:FireServer()
...
local function TouchHandbrake(input, GPE)
	if input.UserInputState == Enum.UserInputState.Begin then
		...
		if _PBrake then
			Handbrake.Frame.Parking.TextColor3 = Color3.fromRGB(255, 85, 127) -- Rojo (Activado)
		else
			Handbrake.Frame.Parking.TextColor3  = Color3.fromRGB(200, 200, 200) -- Verde (Desactivado)
		end

	elseif input.UserInputState == Enum.UserInputState.End then
		if car.DriveSeat.Velocity.Magnitude>5 then
			_PBrake = false
			Handbrake.Frame.Parking.TextColor3  = Color3.fromRGB(200, 200, 200)
		end
	end
end
...
function Steering()
	...
	local sDecay = (1-math.min(car:WaitForChild("DriveSeat").Velocity.Magnitude/_Tune.SteerDecay,1-(_Tune.MinSteer/100)))
...
--Print Version
local ver=require(car:FindFirstChildOfClass("ModuleScript").README)
