...
--[[START]]
repeat task.wait() until script.Parent ~= nil
...
local PGS_ON = workspace:PGSIsEnabled() -- Keep this if you need it for your logic
...
function MakeWeld(x, y, type, s)
	...
	elseif type == "WeldConstraint" then
		W = Instance.new("WeldConstraint")
	else
		warn("Invalid joint type specified. Defaulting to Weld.")
		W = Instance.new("Weld")
	end

	W.Part0 = x
	W.Part1 = y
	W.C0 = x.CFrame:inverse() * x.CFrame -- Local offset for Part0
	W.C1 = y.CFrame:inverse() * x.CFrame -- Local offset for Part1

	-- Parent the weld to one of the parts (x in this case)
	W.Parent = x
...
function ModelWeld(a, b)
	if a:IsA("BasePart") then
		MakeWeld(b, a, "Weld") -- Weld b to a
	elseif a:IsA("Model") then
		for _, v in pairs(a:GetChildren()) do
			ModelWeld(v, b) -- Recursively weld each child
		end
	end
end
...
function UnAnchor(a)
	...
	for _, v in pairs(a:GetChildren()) do
		UnAnchor(v) -- Recursively unanchor each child
	end
end
