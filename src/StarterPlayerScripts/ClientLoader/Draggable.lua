...
function DraggableObject:Enable()
	...
	local function update(input)
		...
		-- Clamp so it stays inside the parent
		newX = math.clamp(newX, 0, parentSize.X - objSize.X)
		newY = math.clamp(newY, 0, parentSize.Y - objSize.Y)

		local newPosition = UDim2_new(startPos.X.Scale, newX, startPos.Y.Scale, newY)
...
Observers.observeTag("Draggable", function(ui: GuiObject)
	if not ui then return end
	local drag = DraggableObject.new(ui)
	drag:Enable()
end)
